...
<div class="main-content" style="padding: 40px; background-color: #f4f7fe;">
    <div class="mb-4">
        ...
        <h2 class="fw-bold mt-2">Chỉnh sửa học kỳ: <?= htmlspecialchars($semester['name']) ?></h2>
    </div>
    ...
    <div class="card border-0 shadow-sm p-4" style="border-radius: 20px; max-width: 600px;">
        <form action="index.php?page=semester-update&id=<?= $semester['id'] ?>" method="POST"> <?= csrfField() ?>
            ...
            <div class="mt-4">
                <button type="submit" class="btn btn-warning rounded-pill px-5 fw-bold text-white shadow-sm" style="border: none;">Cập nhật học kỳ</button>
