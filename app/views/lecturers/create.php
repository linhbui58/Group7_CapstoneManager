<?php require '../app/views/layouts/header.php'; ?>
<?php require '../app/views/layouts/sidebar.php'; ?>

<div class="main-content" style="padding: 30px; background-color: #f8fafc;">
    <div class="mb-4">
        <a href="index.php?page=lecturers" class="text-decoration-none text-muted small"><i class="fa-solid fa-arrow-left me-1"></i> Quay lại</a>
        <h2 class="fw-bold mt-2">Thêm giảng viên mới</h2>
    </div>

    <div class="card border-0 shadow-sm p-4" style="border-radius: 20px; max-width: 600px; border: 1px solid #f1f5f9;">
        <form action="index.php?page=lecturer-store" method="POST"> <?= csrfField() ?>
            <div class="mb-3">
                <label class="form-label small fw-bold text-muted">HỌ VÀ TÊN</label>
                <input type="text" name="full_name" class="form-control rounded-pill px-3" placeholder="Nhập tên giảng viên" required>
            </div>
            <div class="mb-3">
                <label class="form-label small fw-bold text-muted">ĐỊA CHỈ EMAIL (Sẽ tạo tài khoản mới)</label>
                <input type="email" name="email" class="form-control rounded-pill px-3" placeholder="example@vnu.edu.vn" required>
            </div>
            <div class="mb-4">
                <label class="form-label small fw-bold text-muted">CHUYÊN MÔN</label>
                <input type="text" name="expertise" class="form-control rounded-pill px-3" placeholder="Vd: Trí tuệ nhân tạo">
            </div>
            <div class="d-flex gap-2">
                <button type="submit" class="btn btn-primary rounded-pill px-5 fw-bold shadow-sm">Lưu giảng viên</button>
                <a href="index.php?page=lecturers" class="btn btn-light rounded-pill px-4">Hủy</a>
            </div>
        </form>
    </div>
</div>

<?php require '../app/views/layouts/footer.php'; ?>
